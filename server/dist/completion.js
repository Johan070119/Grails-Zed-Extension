...
const methodRe = /^\s*(?:(?:private|protected|public)\s+)?(?:static\s+)?(?:def|\w+)\s+(\w+)\s*\(/gm;
...
function detectContext(lineUpTo, project) {
    // 1. Import
    ...
    // 3. action: "..."
    const actionMatch = lineUpTo.match(/controller\s*:\s*['"](\w+)['"][^)]*action\s*:\s*['"][^'"]*$/);
    if (actionMatch) {
    ...
    if (pkgDomainMatch && project) {
        ...
        if (!isCamelCaseSplit) {
            if (project.controllers.has(candidate + "Controller") ||
                project.controllers.has(candidate)) {
                const ctrlName = project.controllers.has(candidate + "Controller")
                    ? candidate + "Controller"
                    : candidate;
                return { kind: "controller_static", controllerName: ctrlName };
            }
            if (project.services.has(candidate + "Service") ||
            ...
            if (project.domains.has(candidate)) {
                // Detectar findByXAnd...
                const staticAndMatch = lineUpTo.match(/([A-Z]\w*)\.findBy(\w+?)And(\w*)$/);
                if (staticAndMatch) {
                    return {
...
function importCompletions(project) {
    const items = [];
    const add = (artifact) => {
...
function controllerNameCompletions(project) {
    const items = [];
...
function actionNameCompletions(targetController, project, currentFilePath) {
    const items = [];
    let filePath;
    if (targetController) {
        const key = targetController.charAt(0).toUpperCase() +
...
function viewPathCompletions(project, currentFilePath) {
    const items = [];
...
function gormStaticCompletions(domainName, project) {
    ...
    const staticMethods = [
        { label: "list()", insertText: "list()" },
        {
            label: "get(id)",
            insertText: "get(${1:id})",
            format: node_1.InsertTextFormat.Snippet,
        },
    ...
    for (const m of staticMethods) {
        items.push({
            ...
            insertTextFormat: m.format ?? node_1.InsertTextFormat.PlainText,
        });
    }
...
function gormStaticAndCompletions(domainName, andPrefix, project) {
    const domain = project.domains.get(domainName);
    if (!domain)
        return [];
    const items = [];
...
function gormInstanceCompletions(instanceType, project, docSrc) {
    const items = [];
    // Intentar inferir el tipo de la variable desde el documento
    const typeRe = new RegExp("(?:def|[A-Z]\\w*)\\s+" + instanceType + "\\s*(?:=|\\()");
    const inferredMatch = docSrc.match(typeRe);
    let domain;
    if (inferredMatch) {
        // Buscar asignación de tipo: def book = Book.findBy...
        const assignRe = new RegExp(instanceType + "\\s*=\\s*([A-Z]\\w*)\\s*\\.");
        const am = docSrc.match(assignRe);
        if (am)
            domain = project.domains.get(am[1]);
    }
    // Fallback: capitalizar el nombre de variable
    if (!domain) {
        const capitalized = instanceType.charAt(0).toUpperCase() + instanceType.slice(1);
...
function serviceMethodCompletions(serviceName, project) {
    ...
    if (!artifact) {
        ...
        if (!found)
            return [];
        return parseMethods(fs.readFileSync(found.filePath, "utf8")).map((m) => ({
            label: m,
            kind: node_1.CompletionItemKind.Method,
            insertText: m,
        }));
    }
    return parseMethods(fs.readFileSync(artifact.filePath, "utf8")).map((m) => ({
        label: m,
        kind: node_1.CompletionItemKind.Method,
        insertText: m,
    }));
}
function controllerMethodCompletions(controllerName, project) {
...
function artifactNameCompletions(partial, project) {
    const items = [];
...
function renderRedirectKeyCompletions() {
    const keys = [
...
function getCompletions(doc, params, project) {
    const text = doc.getText();
    const offset = doc.offsetAt(params.position);
    const lineStart = text.lastIndexOf("\n", offset - 1) + 1;
    const lineUpTo = text.slice(lineStart, offset);
    if (!project)
        return controllerScopeCompletions();
    ...
    switch (ctx.kind) {
        ...
        case "string_action":
            return actionNameCompletions(ctx.targetController, project, doc.uri.replace(/^file:\/\//, ""));
        case "string_view":
            return viewPathCompletions(project, doc.uri.replace(/^file:\/\//, ""));
        ...
        case "gorm_static_and":
            return ctx.domainName
                ? gormStaticAndCompletions(ctx.domainName, ctx.andPrefix ?? "", project)
                : [];
        case "gorm_instance":
